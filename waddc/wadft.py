...
# 获取已清洗好的全A股列表
stocklist_allA = pd.read_csv('1-stocklist_allA.csv')
...
def get_stock_data(code):
    ...
    df_stock = df_stock[['datetime', 'open', 'high', 'low', 'close', 'volume', 'openinterest', \
                         'news', 'heat', 'turnover_rate_f', 'circ_mv', 'total_mv']]
...
class momentum_factor_strategy(bt.Strategy):
    ...
    def __init__(self):
        # 已清洗过的每日可用股票列表
        self.df_byday = pd.read_csv('3-allA_valid_stocklist_byday.csv')
        # 交易天数，用于判断是否交易
        self.bar_num = 0
        # 上次交易股票的列表
        self.last = []
    ...
    def get_df_fac(self, stocklist, prev=0):
        ...
        df_fac = pd.DataFrame(columns=['code', 'momentum_value', 'turnover_rate_f', 'circ_mv'])
        for stock in stocklist:
            ...
            new = pd.DataFrame({'code': stock, 'momentum_value': stock_momentum,
                                'turnover_rate_f': TR_F, 'circ_mv': CM, }, index=[1])
